...
def all_post(post_id):
    ...
    created_at = Post.query.filter_by(id=post_id).with_entities(Post.created_at).first()
    post_date = created_at[0].date()
    md_to_html = mistune.html(id.content)
    return render_template('post-page.html', post=id, html_content=md_to_html, date=post_date)
